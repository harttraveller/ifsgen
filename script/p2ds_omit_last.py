...
def run_sierpinski_ifs(
    ...
    for iteration_index in range(number_of_iterations):
        ...
        if not (last_edge is None):
            while bool((edge_point == last_edge).all()):
                edge_point = select_edge_point(edge_points)
        last_edge = edge_point
...
def main(iterations: int = 1_000_000) -> None:
    ...
    alpha = min(1, 1000 / iterations + 0.5)
    ax.scatter(*run_sierpinski_ifs(iterations), alpha=alpha, s=size)
    plt.show()
